...
impl ArrayMap {
    ...
    pub fn new() -> Self{
        Self{
            ...
            module: Module{
                reg: Regex::new(r#"[^A-Za-z0-9_]{1}array_map\(((?:[^\(\)]+|\((?1)\))*)\)"#).unwrap(),
                replace_params_index: None,
                exclude_params_index: Some(vec![1])
            }
       }
    }
...
impl Upgrade for ArrayMap{
    
    fn view(&mut self, content: &String) -> bool {
        match transform(&self.module, content, ArrayMap::parse_match) {
            Some(v) => {
