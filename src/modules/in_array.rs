...
impl InArray {
    ...
    pub fn new() -> Self{
        Self{
            ...
            module: Module{
                reg: Regex::new(r#"[^A-Za-z0-9_]{1}in_array\(((?:[^\(\)]+|\((?1)\))*)\)"#).unwrap(),
                replace_params_index: Some(vec![2]),
...
impl Upgrade for InArray{
    
    fn view(&mut self, content: &String) -> bool {
        match transform(&self.module, content, InArray::parse_match) {
            Some(v) => {
